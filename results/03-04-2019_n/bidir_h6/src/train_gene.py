...
def train_iter_gene():
    cfg = config.Config()
    data_ob_gene = DataPrepGene(cfg, mode=mode)
    ...
    for epoch_num in range(cfg.num_epochs):
        ...
        iter_num = 0
        hidden_states = np.zeros((4, cfg.hidden_size_encoder * cfg.numdir))
        # hidden_states = []
        rec_loss = 0
        encoder_init = True
        decoder_init = True
        ...
        try:
            for track_cut in data_gen_train:
                ...
                rec_loss, hidden_states, encoder_init, decoder_init, predicted_cut = unroll_loop(cfg, track_cut, model,
                                                                                                 encoder_optimizer,
                                                                                                 decoder_optimizer,
                                                                                                 criterion,
                                                                                                 hidden_states,
...
def unroll_loop(cfg, track_cut, model, encoder_optimizer,
    ...
    if encoder_init:
        ...
        encoder_hidden = Variable(torch.from_numpy(encoder_hidden_init).float().unsqueeze(0).unsqueeze(0)).view(-1, 1,
                                                                                                                3).cuda(
            gpu_id)
    ...
    encoder_outputs = Variable(torch.zeros(nValues, 1, cfg.hidden_size_encoder * cfg.numdir)).cuda(gpu_id)
    encoder_states = Variable(torch.zeros(nValues * cfg.numdir, 1, cfg.hidden_size_encoder)).cuda(gpu_id)
    ...
    for ei in range(0, nValues):
        ...
        encoder_states[2 * ei] = encoder_state[0][0]
        encoder_states[2 * ei + 1] = encoder_state[1][0]

    hidden_states[0] = encoder_hidden.squeeze(0).flatten().cpu().data.numpy()
    ...
    for di in range(0, nValues):
        decoder_state = encoder_states[2 * di:2 * (di + 1)]
    ...
    return mean_loss, hidden_states, encoder_init, decoder_init, predicted_cut
