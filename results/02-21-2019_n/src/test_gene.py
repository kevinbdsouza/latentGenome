...
def test_gene(cfg):
    ...
    try:
        for track_cut in data_gen_test:
            ...
            mse, hidden_states, encoder_init, predicted_cut = testing_loop(cfg, track_cut, model,
...
def testing_loop(cfg, track_cut, model, hidden_states, encoder_init):
    ...
    hidden_states = encoder_hidden.squeeze(0).cpu().data.numpy()
    ...
    for di in range(0, nValues):
        decoder_state = encoder_hidden_states[di].unsqueeze(0)
    ...
    return mse / nValues, hidden_states, encoder_init, predicted_cut


if __name__ == '__main__':
    setup_logging()

    model_dir = '/home/kevindsouza/Documents/projects/latent/results/2019-02-22/model'
