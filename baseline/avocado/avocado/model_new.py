...
class Avocado(object):
	"""An Avocado multi-scale deep tensor factorization train_fns.

	The Avocado train_fns is a multi-scale deep tensor factorization train_fns. It is
	...
	train_fns can impute epigenomic assays that have not yet been performed, and
	...
	n_genomic_positions : int, optional
		The number of genomic positions to train_fns. This is typically either
	...
	n_layers : int, optional
		The number of hidden layers in the neural train_fns. Default is 2.
	...
	>>>
	>>> train_fns = Avocado(celltypes, assays)
	>>> train_fns.fit(data)
	>>>
	>>> track = train_fns.predict("E065", "H3K27me3")
	"""
	...
	def celltype_embedding(self):
		...
		raise ValueError("No layer in train_fns named 'celltype_embedding'.")
	...
	def assay_embedding(self):
		...
		raise ValueError("No layer in train_fns named 'assay_embedding'.")
	...
	def fit_celltypes(self, X_train, X_valid=None, n_epochs=200, epoch_size=120,
		verbose=1, callbacks=None, **kwargs):
		"""Add a new cell type(s) to an otherwise frozen train_fns.

		This method will add a new cell type to the cell type embedding after
		freezing all of the other parameters in the train_fns, including weights
		and the other cell type positions. Functionally it will train a new
		cell type embedding and return a new train_fns whose cell type embedding
		...
		appear in the train_fns.celltypes attribute but the assays should exclusively
		appear in the train_fns.assays attribute.
	...
	def fit_assays(self, X_train, X_valid=None, n_epochs=200, epoch_size=120,
		verbose=1, callbacks=None, **kwargs):
		"""Add a new assay(s) to an otherwise frozen train_fns.
		...
		assay embedding and return a new train_fns whose assay embedding
		...
		appear in the train_fns.assays attribute, but the cell types should appear
		in the train_fns.celltypes attribute.
	...
	def predict(self, celltype, assay, verbose=0):
		...
		celltype : str
			...
			elements from the list of cell types passed in upon train_fns
			initialization.

		assay : str
			The assay to be imputed. Must be one of the elements from the list
			of assays passed in upon train_fns initialization.
	...
	def save(self, name="avocado", separators=(',', ' : '), indent=4):
		"""Serialize the train_fns to disk.

		This function produces two files. The first is a json file that has the
		train_fns hyperparameters associated with it. The second is a h5 file that
		contains the architecture of the neural network train_fns, the weights, and
		the optimizer.
	...
	def load_weights(self, name, verbose=0):
		...
		Load the weights of a pre-saved train_fns on a layer-by-layer case. This
		method will iterate through the layers of the serialized train_fns and
		this train_fns jointly and set the weights in this train_fns to that of the
		serialized train_fns should the weight matrices be of the same size. Should
		they not be of the same size it will not modify the current weight
		matrix. 

		A primary use of this function should be after an initial train_fns has been
		trained on the Pilot regions and now one is fitting a train_fns to each of
		...
		for i, (self_layer, layer) in enumerate(zip(self.model.layers, model.layers)):
			...
			if verbose > 0:
				print("{} has been set from serialized train_fns".format(name))

	@classmethod
	def load(self, name, freeze_celltypes=False, freeze_assays=False,
		freeze_genome_25bp=False, freeze_genome_250bp=False, 
		freeze_genome_5kbp=False, freeze_network=False):
		"""Load a train_fns that has been serialized to disk.

		The keras train_fns that is saved to disk does not contain any of the
		wrapper information 
